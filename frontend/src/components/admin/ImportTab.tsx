...
import {
  ...
  saveBaiduOcr, saveDeepSeekConfig, saveCardConfig,
  addImageCard
} from '@/lib/api'
...
export default function ImportTab() {
  ...
  // GPT 视觉模型配置（CARD_API_KEY）
  const [showGptForm, setShowGptForm] = useState(false)
  const [gptApiKey, setGptApiKey] = useState('')
  const [gptBaseUrl, setGptBaseUrl] = useState('https://api.apimart.ai/v1')
  ...
  async function handleImport() {
    ...
    try {
      ...
      if (mode === 'text') {
        ...
        r = await importImage(Number(chapterId) || 1, file, source, 'baidu')
  ...
  async function handleSaveGpt() {
    ...
    setGptMsg('')
    try {
      await saveCardConfig({ api_key: gptApiKey, base_url: gptBaseUrl, model: gptModel })
      setGptMsg('配置保存成功！')
      ...
      setGptMsg(e?.response?.data?.detail || '保存失败')
    } finally {
      setGptSaving(false)
    }
  }
  ...
  return (
    <div className="space-y-6">
          ...
          <div>
            <div className="mb-3">
              ...
              <div className="space-y-2">
                {/* 百度 OCR（唯一引擎） */}
                <div className="flex items-center gap-3 p-3 rounded-lg border border-blue-500 bg-blue-50">
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <span className={`text-xs ${ocrConfig?.baidu_configured ? 'text-green-600' : 'text-orange-500'}`}>
        ...
        <div className="bg-green-50 border border-green-200 rounded-xl p-4 text-sm">
          ...
          {result.status === 'duplicate' ? (
            ...
            <div className="text-green-600">
              <div>
                {mode === 'image_card' ? '已生成图片总结卡：' : '已归纳整合为大知识点：'}
                <span className="font-medium">{result.title}</span>
              </div>
              {result.raw_points_count != null && (
                <div className="text-xs text-gray-500 mt-1">
                  {result.engine === 'gpt' ? 'GPT' : 'DeepSeek'} 识别 {result.raw_points_count} 条原始要点 → 已归纳整合为 1 个大知识点
                </div>
              )}
              {(result.tags?.length > 0 || result.difficulty) && (
                <div className="mt-2 flex flex-wrap gap-1 items-center text-xs">
                  ...
                  {result.difficulty && (
                    <span className="px-2 py-0.5 bg-amber-100 text-amber-700 rounded">难度 {result.difficulty}/5</span>
                  )}
                </div>
              )}
              {(result.content || result.content_preview) && (
                <div className="mt-2 text-gray-700 text-xs bg-white p-3 rounded border max-h-96 overflow-auto whitespace-pre-wrap font-mono leading-relaxed">
                  {result.content || result.content_preview}
                </div>
              )}
              {result.raw_points?.length > 0 && (
                <details className="mt-2 text-xs">
                  <summary className="cursor-pointer text-gray-500 hover:text-gray-700">
                    查看原始拆分的 {result.raw_points.length} 条要点
                  </summary>
                  <div className="mt-1 space-y-1 bg-gray-50 p-2 rounded border max-h-64 overflow-auto">
                    {result.raw_points.map((p: any, i: number) => (
                      <div key={i} className="border-b last:border-b-0 pb-1 last:pb-0">
                        <div className="font-medium text-gray-700">{i + 1}. {p.title}</div>
                        <div className="text-gray-500 mt-0.5 whitespace-pre-wrap">{p.content}</div>
                      </div>
                    ))}
                  </div>
                </details>
      ...
      {/* 图片AI识别配置：仅图片总结卡模式显示（GPT-4o-mini 仅用于总结卡） */}
      {mode === 'image_card' && (
      <div className="bg-white rounded-xl shadow p-6">
        <h3 className="font-bold text-gray-700 mb-3">图片 AI 识别配置</h3>

        {/* GPT-4o-mini 配置（CARD_API_KEY） */}
        <div className="mb-4">
          <div className="flex items-center justify-between mb-1">
            <div>
              <div className="text-sm text-gray-600">
                GPT-4o-mini：<span className="font-mono text-blue-700">{ocrConfig?.card_model || 'gpt-4o-mini'}</span>
                <span className="ml-2 text-xs text-gray-400">（{ocrConfig?.card_base_url || 'https://api.apimart.ai/v1'}）</span>
              </div>
              <div className="text-xs text-gray-400 mt-0.5">
                {ocrConfig?.card_configured
                  ? `✓ 已配置 ${ocrConfig.card_api_key_preview}`
                  : '⚠ 未配置 API Key，GPT-4o-mini 视觉识别不可用'}
              </div>
            </div>
            <button type="button" onClick={() => setShowGptForm(v => !v)}
              className="text-xs text-blue-600 border border-blue-300 rounded px-2 py-0.5 hover:bg-blue-50">
              {showGptForm ? '收起' : '配置'}
            </button>
          </div>
          {gptMsg && (
            <div className={`text-xs mb-2 ${gptMsg.includes('成功') ? 'text-green-600' : 'text-red-500'}`}>{gptMsg}</div>
          )}
          {showGptForm && (
            <div className="border rounded-lg p-4 bg-gray-50 space-y-3">
              <div>
                <label className="text-xs text-gray-500 block mb-1">快速选择预设</label>
                <div className="flex gap-2 flex-wrap">
                  <button type="button"
                    onClick={() => { setGptBaseUrl('https://api.apimart.ai/v1'); setGptModel('gpt-4o-mini') }}
                    className={`px-3 py-1.5 rounded-lg text-xs border transition-colors ${
                      gptModel === 'gpt-4o-mini' ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-gray-600 border-gray-300 hover:border-blue-400'
                    }`}>GPT-4o-mini</button>
                  <button type="button"
                    onClick={() => { setGptBaseUrl('https://api.openai.com/v1'); setGptModel('gpt-4o') }}
                    className={`px-3 py-1.5 rounded-lg text-xs border transition-colors ${
                      gptModel === 'gpt-4o' ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-gray-600 border-gray-300 hover:border-blue-400'
                    }`}>GPT-4o</button>
                </div>
              </div>
              <div>
                ...
                <input type="password" className="border rounded-lg px-3 py-2 w-full text-sm"
                  placeholder="输入 GPT API Key（sk-...）"
                  value={gptApiKey} onChange={e => setGptApiKey(e.target.value)} />
              </div>
              <div>
                ...
                <input className="border rounded-lg px-3 py-2 w-full text-sm font-mono"
                  value={gptBaseUrl} onChange={e => setGptBaseUrl(e.target.value)} />
              </div>
              <div>
                <label className="text-xs text-gray-500 block mb-1">模型名称</label>
                <input className="border rounded-lg px-3 py-2 w-full text-sm font-mono"
                  value={gptModel} onChange={e => setGptModel(e.target.value)} />
              </div>
              <button onClick={handleSaveGpt} disabled={gptSaving || !gptApiKey}
                className="bg-blue-600 text-white px-4 py-1.5 rounded-lg text-sm hover:bg-blue-700 disabled:opacity-50">
                {gptSaving ? '保存中...' : '保存'}
              </button>
            </div>
