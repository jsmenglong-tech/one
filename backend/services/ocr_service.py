...
def _get_gpt_client():
    """获取 GPT 视觉模型 OpenAI 客户端（CARD_API_KEY）"""
    from openai import AsyncOpenAI
    settings = get_settings()
    if not settings.card_api_key:
        raise RuntimeError("未配置 GPT 视觉模型 API Key，请在系统配置 → 图片AI识别中填写")
    return AsyncOpenAI(
...
async def extract_and_split_by_gpt(image_bytes: bytes) -> list[dict]:
    """使用 GPT-4o-mini（CARD_API_KEY）直接识别图片并拆分为知识点列表"""
    import json
    import re
    settings = get_settings()
    ...
    try:
        ...
        raise RuntimeError(f"GPT视觉API调用异常: {type(e).__name__}: {e}")
    ...
    try:
        ...
        raise RuntimeError(f"GPT视觉识别返回格式无法解析: {content[:300]}")


async def extract_and_consolidate_by_gpt(image_bytes: bytes) -> tuple[dict, list[dict]]:
    """两阶段：① GPT 视觉识别+拆分（CARD_API_KEY） ② DeepSeek 归纳整合（LLM_API_KEY）
    返回 (consolidated, raw_points)，方便上层展示原始拆分结果"""
    from services.llm_service import consolidate_points
    raw_points = await extract_and_split_by_gpt(image_bytes)
    if not raw_points:
        raise RuntimeError("未从图片中识别到任何知识点")
    consolidated = await consolidate_points(raw_points)
    return consolidated, raw_points
...
async def describe_image_for_card(image_bytes: bytes) -> dict:
    ...
    api_key = settings.card_api_key
    if not api_key:
        raise RuntimeError("未配置图片总结卡 API Key，请在系统配置中填写")
    client = AsyncOpenAI(
        api_key=api_key,
        base_url=settings.card_base_url,
    )
